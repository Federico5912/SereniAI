...
// artifacts/sereni/netlify/functions/chat.mjs
var chat_exports = {};
...
// artifacts/sereni/node_modules/@netlify/runtime-utils/dist/main.js
...
// artifacts/sereni/node_modules/@netlify/otel/dist/main.js
...
// artifacts/sereni/node_modules/@netlify/blobs/dist/chunk-YAGWSQMB.js
...
// artifacts/sereni/node_modules/@netlify/blobs/dist/main.js
...
var STORE_NAME = "sereniai-conversations";
var crisisPatterns = [
  /suicide/i,
  /kill myself/i,
  /self[-\s]?harm/i,
...
function getTextFromResponse(data) {
  ...
  const chunks = Array.isArray(message?.content) ? message.content.map((item) => {
...
async function handler(event) {
  ...
  try {
    ...
    const latestUserMessage = [...messages].reverse().find((message) => message?.role === "user" && typeof message.text === "string");
    ...
    if (crisisPatterns.some((pattern) => pattern.test(latestUserMessage.text))) {
      reply = "I\u2019m really glad you said that out loud. If you may hurt yourself or are in immediate danger, contact emergency services or a local crisis line right now and stay near another person if you can. If you want, send one short message to someone you trust: 'I need support right now and I should not be alone.'";
      source = "safety-fallback";
    } else {
      const conversation = messages.filter(
        (message) => (message?.role === "user" || message?.role === "assistant") && typeof message.text === "string" && message.text.trim()
      ...
      if (!response.ok) {
        const message = data?.error?.message || "OpenAI request failed. Check logs and API configuration.";
