...
// artifacts/sereni/netlify/functions/conversations.mjs
  ...
  try {
    ...
    if (event.httpMethod === "GET") {
      ...
      const conversations = await Promise.all(
        blobs.map(
          async (blob) => store.get(blob.key, {
            type: "json"
          })
        )
      );
      const items = conversations.filter(Boolean).sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()).map((conversation) => ({
        id: conversation.id,
        title: conversation.title,
        mood: conversation.mood,
        updatedAt: conversation.updatedAt,
        preview: conversation.messages?.find((message) => message.role === "user")?.text?.slice(0, 90) || "No messages yet."
      }));
